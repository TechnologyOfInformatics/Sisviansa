...
modificar_grupo_usuario() {
  ...
  echo “Se agregara al usuario $nombre_de_usuario al grupo $nombre_de_grupo”
...
#Ésto lo saque de chatgpt directamente
animacion() {
  local barra="-\\|/"
  local index=0

  while :; do
    printf "\rPresione cualquier tecla para continuar... %c " "${barra:$index:1}"
    sleep 0.1
    index=$(( (index + 1) % 4 ))
  done
}
